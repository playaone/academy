...
def test_service_rejects_duplicate_title(repository):
    ...
    service.create_project({"title": "Existing project", "description": "Test data"})

    with pytest.raises(ConflictError, match="A project with this title already exists"):
        service.create_project(
            {"title": "Existing project", "description": "Test data"}
        )
...
def test_service_update_project_successful(repository):
    ...
    created = service.create_project(
        {"title": "Create Title", "description": "Description for created project"}
    )

    project_id = created.id

    updated = service.update_project(project_id, {"title": "Updated title"})
...
def test_service_detetion_successful(repository):
    ...
    create_project = service.create_project(
        {"title": "Created Project Title", "description": "Created Project Description"}
    )
